...
//############################################################################
/*##             Usuário e Senha para login no Overleaf                   ##*/
//############################################################################

const usuario = 'user@email.com.br'     // Insira o email de acesso ao Overleaf
const senha = '***********'             // Insira a senha de acesso ao Overleaf
...
it ('Automatização de testes: Overleaf', () => {
    ...
    // Excluindo arquivo
    cy.get('#left-menu-mask').click()
    cy.get('.toolbar-header-back-projects').click()
    cy.get('.form-control').type('Modelo INATEL TCC GRADUACAO')
    cy.get('.project-list-table-header-row > .project-list-table-name-cell > .project-list-table-name-container > .project-list-table-select-item').click()
    cy.get('[ng-if="filter !== \'trashed\'"]').click({force:true})
